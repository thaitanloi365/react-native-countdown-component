...
class CountDown extends React.Component {
  ...
  renderDigit = d => {
    const {
      ...
      size,
      digitWidthRatio = 2.3,
      digitHeightRatio = 2.6
    } = this.props;
    const width = digitWidthRatio * size;
    const height = digitHeightRatio * size;
    return (
      <View style={[styles.digitCont, { width, height }, digitStyle]}>
  ...
  renderLabel = label => {
    ...
    if (label) {
      return (
        <Text style={[styles.timeTxt, { fontSize: size / 1.8 }, timeLabelStyle]}>{label}</Text>
      );
    }
  };
